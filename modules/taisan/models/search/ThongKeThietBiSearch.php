 ...
 */
class ThongKeThietBiSearch extends ThietBi
{
    public $tuNgay;
    public $denNgay;
    public $coPhieuMuaSam;
    public $coNgayMua;
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            ...
            [['ma_thiet_bi', 'ten_thiet_bi', 'nam_san_xuat', 'serial', 'model', 'xuat_xu', 'dac_tinh_ky_thuat', 'ngay_mua', 'han_bao_hanh', 'ngay_dua_vao_su_dung', 'trang_thai', 'ngay_ngung_hoat_dong', 'ghi_chu', 'thoi_gian_tao', 'tuNgay', 'denNgay', 'coPhieuMuaSam', 'coNgayMua'], 'safe'],
        ];
    }
        ...
        if($cusomSearch != NULL){
            ...
            $query->andFilterWhere(['like', 'ma_thiet_bi', $this->ma_thiet_bi])
                ...
                if($this->tuNgay && $this->denNgay){
                    $this->tuNgay = $custom->convertDMYToYMD($this->tuNgay);
                    $this->denNgay = $custom->convertDMYToYMD($this->denNgay);
                    $query->andWhere(['>=','ngay_mua', $this->tuNgay]);
                    $query->andWhere(['<=','ngay_mua', $this->denNgay]);
                } else if($this->tuNgay && !$this->denNgay){
                    $this->tuNgay = $custom->convertDMYToYMD($this->tuNgay);
                    $query->andWhere(['>=','ngay_mua', $this->tuNgay]);
                } else if(!$this->tuNgay && $this->denNgay){
                    $this->denNgay = $custom->convertDMYToYMD($this->denNgay);
                ...
                if($this->coPhieuMuaSam && $this->coPhieuMuaSam==true){
                    $query->andWhere('id_phieu_mua_sam IS NOT NULL');
                }
                
        }
        
        //ngay mua not null
        if($this->coNgayMua && $this->coNgayMua==true){
            $query->andWhere('ngay_mua IS NOT NULL');
        }
